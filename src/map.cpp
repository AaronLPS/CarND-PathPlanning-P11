...
void Map::read(string map_file) {
    ...
    // Load up map values for waypoint's x,y,s and d unit normal vector (split up into the x component, and the y component).
...
//    cout << "len_ref=" << len_ref << endl;
    ...
    //For the following test, precomputed------
	double frenet_s = 0.0;
    map_s.push_back(0.0);
...
vector<double> Map::getFrenet(double x, double y, double theta) {
  ...
  vector<double> &maps_y = this->new_map_waypoints_y;
    ...
    double center_x = TRACK_CENTER_X - maps_x[prev_wp];
    double center_y = TRACK_CENTER_Y - maps_y[prev_wp];
